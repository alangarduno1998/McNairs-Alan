...
def displayrej(rejectedImgPoints):
    rej2 = np.array([r[0] for r in rejectedImgPoints])
    data1 = pd.DataFrame({"x": rej2[:, :, 0].flatten(), "y": rej2[:, :, 1].flatten()},
                    index=pd.MultiIndex.from_product([ids.flatten(),
                                                      ["r{0}".format(i)for i in np.arange(3)+1]], names=["marker", ""]))
    data1 = data1.unstack().swaplevel(0, 1, axis=1).stack()
    data1["m1"] = data1[["r1"]].mean(axis=1)
    data1["m2"] = data1[["r2", "r3"]].mean(axis=1)
    data1["m3"] = data1[["r3", "r1"]].mean(axis=1)
    data1["o"] = data1[["m1", "m2", "m3"]].mean(axis=1)
